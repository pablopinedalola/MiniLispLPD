...
-- Resta y división (mínimo 1/2 args como en Racket/Scheme)
...
-- Comparadores n-arios (cadenas adyacentes)
...
desugarApp (S.SVar "!=") as =
  chainAnd [ C.CNe (desugar ai) (desugar aj)
           | (i,ai) <- ixs, (j,aj) <- ixs, i < j ]
  where
    ixs :: [(Int, S.SExpr)]
    ixs = zip [0..] as
...
-- Plegado binario para variádicos (exige ≥2 args para + y *)
